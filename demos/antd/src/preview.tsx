...
const SamplePreview = () => {
  ...
  async function init() {
    ...
    const components = await injectComponents(
      buildComponents(libraryMap, componentsMap, (schema) => {
        return mergeWith(schema, pageSchema, customizer);
      }),
    );
...
ReactDOM.createRoot(document.getElementById('ice-container')!).render(
  <SamplePreview />
);
